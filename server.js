...
app.get("/api/get_test/:tid",function(req, res){
    let query = 'select s.sid, s.firstname, s.lastname, k.kid, k.klasse, e.note, e.kommentar, t.tid, t.bezeichnung, date_format(t.datum,"%d-%m-%Y") as datum, f.fid, f.fach, l.lid,'+
    ' l.firstname as lfirstname, l.lastname as llastname ' + 
                'from schueler as s join ergebnisse as e join tests as t join klassen'+
                ' as k join faecher as f join lehrer as l ' +
                'on e.sid = s.sid and e.tid = t.tid and s.kid =  t.kid ' +
                'and k.kid = s.kid and f.fid = t.fid and l.lid = t.lid where t.tid = ' + req.params.tid; 
